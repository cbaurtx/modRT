#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# testModbusInvalidFunction.py
#
# Copyright (C) 2015 Christof Baur
#
# This software may be modified and distributed under the terms
# of the MIT license.  See the license.MIT file for details.
#

import serial
import os

crc16tableHigh = (

)


crc16tableLow = (

)

badFunctionCodes = (
,
)

if os.name == "posix":
    ser = serial.Serial(
        port='/dev/ttyUSB0',
        baudrate=19200,
        parity='E',
        stopbits=serial.STOPBITS_ONE,
        bytesize=serial.EIGHTBITS,
        timeout=0.01
        )

else:   # windows
    ser = serial.Serial(
        port='COM5',
        baudrate=19200,
        parity='E',
        stopbits=serial.STOPBITS_ONE,
        bytesize=serial.EIGHTBITS,
        timeout=0.01
        )

print(ser)

count = 0
crc16error = 0
modbusExceptions = 0
received = ""
TableInd = 0
tableInd255count = 0
badCRCs = []



while count <= len(badFunctionCodes):
    crc16calc = 0xffff
    crc16calcLow = 0xff
    crc16calcHigh = 0xff
    count += 1
    print("\rCount: ",count)

    functionCode  = badFunctionCodes[count % len(badFunctionCodes)] 
    print ("Testing function code: ",hex(functionCode))
    sendThis =  b'\x03' + bytes([functionCode]) + b'\x00\x00' 
    
    # calc CRC 16 of data to send
    for ch in sendThis:
        TableInd = ch ^ crc16calcLow
        crc16calcLow = crc16calcHigh ^ crc16tableHigh[TableInd]
        crc16calcHigh = crc16tableLow[TableInd]
        
    sendThis +=(crc16calcLow.to_bytes(1,"little"))
    sendThis +=(crc16calcHigh.to_bytes(1,"little"))
    print ("TX: ",sendThis)
   
    ser.write(sendThis)
    sendThis = sendThis[:-2]

    print("Wait RX")
    received = ser.readline()
    if (received[1] > 0xf):
        modbusExceptions +=1
        print ("Modbus slave exceptions:", sendThis, received)

    crc16read = received[-2] + (received[-1] << 8)
    print ("CRC16 read = ", hex(crc16read))
    crc16calcLow = 0xff
    crc16calcHigh = 0xff

    # calc CRC 16 of received data
    for ch in received[:-2]:
        TableInd = ch ^ crc16calcLow
        crc16calcLow = crc16calcHigh ^ crc16tableHigh[TableInd]
        crc16calcHigh = crc16tableLow[TableInd]

    crc16calc = crc16calcLow + (crc16calcHigh << 8) 

    print ("CRC16 calc = ", hex(crc16calc))
    if crc16calc != crc16read:
        crc16error += 1
        badCRCs.append(hex(crc16calcHigh))
        badCRCs.append(hex(crc16calcLow))
        badCRCs.append(hex(crc16calc))
        badCRCs.append(hex(crc16read))
        badCRCs.append(str(count))

    print("Received: ",received)
    print ("Modbus slave exceptions: ", modbusExceptions)
    print ("crc16 errors: ", crc16error)
   