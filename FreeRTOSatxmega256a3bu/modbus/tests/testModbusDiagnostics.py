#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# testModbusDiagnostics.py
#
# Copyright (C) 2015 Christof Baur
#
# This software may be modified and distributed under the terms
# of the MIT license.  See the license.MIT file for details.
#
# Test modbus function 0x08


import serial
import os

diagnosticSubfunctions = (
(0, 0x55, 0x11, 0x22,0x44),
(2, 0x00, 0x00), 
(11, 0x00, 0x00),
(12, 0x00, 0x00),
(13, 0x00, 0x00),
(14, 0x00, 0x00),
(15, 0x00, 0x00),
(16, 0x00, 0x00),
(17, 0x00, 0x00),
(18, 0x00, 0x00),
(20, 0x00, 0x00))

crc16tableHigh = (

)


crc16tableLow = (

)

if os.name == "posix":
    ser = serial.Serial(
        port='/dev/ttyUSB0',
        baudrate=19200,
        parity='E',
        stopbits=serial.STOPBITS_ONE,
        bytesize=serial.EIGHTBITS,
        timeout=0.01
        )

else:   # windows
    ser = serial.Serial(
        port='COM5',
        baudrate=19200,
        parity='E',
        stopbits=serial.STOPBITS_ONE,
        bytesize=serial.EIGHTBITS,
        timeout=1
        )

print(ser)

count = -1
error = 0
crc16error = 0
received = ""
TableInd = 0
tableInd255count = 0
badCRCs = []
while count < len(diagnosticSubfunctions) -1:
    crc16calcLow = 0xff
    crc16calcHigh = 0xff
    count += 1
    print("\rCount: ",count)
    print ("Subfuntion: ", diagnosticSubfunctions[count])
    print (bytes(diagnosticSubfunctions[count]))        
    
    sendThis =  b'\x03\x08\x00' + bytes(diagnosticSubfunctions[count])    
    
    # calc CRC 16 of data to send
    for ch in sendThis:
        TableInd = ch ^ crc16calcLow
        crc16calcLow = crc16calcHigh ^ crc16tableHigh[TableInd]
        crc16calcHigh = crc16tableLow[TableInd]
        
    crc16calc = crc16calcLow + (crc16calcHigh << 8)     
    sendThis +=(crc16calcLow.to_bytes(1,"little"))
    sendThis +=(crc16calcHigh.to_bytes(1,"little"))
       
    print ("TX: ",sendThis)
   
    ser.write(sendThis)
    # time.sleep(0.2)
    sendThis = sendThis[:-2]


    print("Wait RX")
    received = ser.read(0xff)
    if (received[1] > 0xf):
        error += 1
        print ("Modbus slave exceptions:", sendThis, received)

    crc16read = received[-2] + (received[-1] << 8)
    print ("CRC16 read = ", hex(crc16read))
    crc16calc = 0xffff
    crc16calcLow = 0xff
    crc16calcHigh = 0xff

    # calc CRC 16 of received data
    for ch in received[:-2]:
        TableInd = ch ^ crc16calcLow
        crc16calcLow = crc16calcHigh ^ crc16tableHigh[TableInd]
        crc16calcHigh = crc16tableLow[TableInd]

    crc16calc = crc16calcLow + (crc16calcHigh << 8) 

    print ("CRC16 calc = ", hex(crc16calc))
    if crc16calc != crc16read:
        crc16error += 1
        badCRCs.append(hex(crc16calcHigh))
        badCRCs.append(hex(crc16calcLow))
        badCRCs.append(hex(crc16calc))
        badCRCs.append(hex(crc16read))
        badCRCs.append(str(count))

    print("Received: ",received)
    print ("Modbus slave exceptions: ", error)
    print ("crc16 errors: ", crc16error)
   